...
For **HeadHunter**, use the native API instead of `web_search`. The autonomous pipeline lives in `src/` (`python3 -m src.pipeline`); a scraper template is in `templates/hh_vacancy_scraper.py`. The pipeline includes cover-letter generation (see `references/hh-cover-letter-generation.md`) and auto-apply via `/negotiations`.
...
- **HeadHunter user OAuth: `invalid_grant` on refresh means re-authorize, not debug the token.** The Playwright app-token renewal flow has been removed. Tokens now come from `scripts/hh_oauth_manager.py` (PKCE authorization_code flow → `data/hh_tokens.json`). If `refresh` returns `invalid_grant`, the refresh_token is expired/revoked — re-run `link` + `exchange`. If `api_request` gets `403 oauth`, `src/auth.py` auto-refreshes once; a second 403 is fatal + Telegram alert. See `references/hh-token-types-and-revocation.md` and `docs/DEPLOY.md`.
- **Token value copied from UI may contain ambiguous characters.** HH.ru OAuth tokens (`USERP…`/`USERID…`) are long strings of uppercase letters and digits. Common mistakes: `0` (digit) vs `O` (letter), `1` vs lowercase `l`, `I` (uppercase i) vs `l`. Always verify with a strict string comparison between the env-файл and what `hh_oauth_manager.py exchange` wrote. This character-ambiguity was the root cause of a real 403 `bad_authorization` incident on 2026-07-03. See `references/hh-token-types-and-revocation.md`.
